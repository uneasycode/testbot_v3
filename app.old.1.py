# app.py - IMAGE BUG FIX VERSION + Multiple Images Support
...
# FIXED: Removed HTML encoded characters
...
def submit_edit(keyword):
    """Process edit response form submission - UPDATED with multiple-images support"""
    logger.info(f"We are in app.route /edit/{keyword} POST")
    response_type = request.form.get('type')

    # FIXED: Proper content handling for different response types + NEW multiple-images support
    if response_type == 'text':
        content = request.form.get('content', '').split('\n')
        content = [line.strip() for line in content if line.strip()]

    elif response_type == 'image':
        # EXISTING: Single image selection (unchanged)
        content = request.form.get('image_file', '') # Use specific field for image
        if not content:
        ...
        content = request.form.getlist('image_files')  # Get list of selected images
        if not content:
            flash("Please select at least one image file for multiple-images", "error")
            return redirect(url_for('edit_response', keyword=keyword))

    elif response_type == 'audio':
        # For audio responses, content should be the selected audio filename
        content = request.form.get('audio_file', '') # Use specific field for audio
        if not content:
    ...
    try:
        ...
        if response_type == 'multiple-images':
            flash(f"Response for '{keyword}' updated with {len(content)} images (all will be sent together)", "success")
        else:
...
def submit_add():
    """Process add response form submission - UPDATED with multiple-images support"""
    ...
    try:
        ...
        if response_type == 'multiple-images':
            flash(f"Response for '{keyword}' added with {len(content)} images (all will be sent together)", "success")
        else:
...
def start_bot_api():
    ...
    # FIXED: Check if bot_thread exists before checking if it's alive
